...
Figure~\ref{fig:em_struggles} demonstrates the effects of these issues with some simple comparisons. When the mixing components are unbalanced or there are significantly overlapping distributions (simulations represented by green lines), the EM algorithm tends to get stuck in slow-convergence areas, likely near saddle points. In the easier settings (red lines), the EM algorithm moves beyond saddle points much more quickly. In both of these cases the EM algorithm eventually escapes and converges to the maximum, but the convergence rate is severely effected.
...
Figure~\ref{fig:algcomps} shows the results of EM, DAEM, and SEM runs using the same initial parameter values for each algorithm.\footnote{SEM is estimated using an initial temperature of $T_0 = 100$ and a cooling schedule of $\alpha = 0.992$.} We see that the EM algorithm exhibits the same issues described in Section~\ref{subsc:em_issues}, where the algorithm is caught in a slow-convergence area for a significant number of iterations. As expected, DAEM has slower initial convergence due to exploration of the state space, but avoids slow-convergence areas, and has better overall convergence performance.
